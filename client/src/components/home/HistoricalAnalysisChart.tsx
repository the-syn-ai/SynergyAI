...
export default function HistoricalAnalysisChart({ websiteId, websiteUrl }: HistoricalAnalysisChartProps) {
  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      setLoading(true);
      try {
        const data = await apiRequest<HistoricalAnalysis[]>(`/api/websites/${websiteId}/historical`);
        if (data) {
  ...
  const createDemoAnalysis = async () => {
    ...
    try {
      ...
      const result = await apiRequest<HistoricalAnalysis>(`/api/websites/${websiteId}/historical`, {
        method: 'POST',
        body: JSON.stringify(demoData),
      });
      
      if (result) {
        setHistoricalData(prev => [...prev, result]);
        toast({
          title: "Success",
